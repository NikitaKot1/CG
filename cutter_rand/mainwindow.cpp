...
void MainWindow::appendDotClipper(int x1, int y1)
{
    ...
    displayImage();
    ui->spinBoxParallel->setMaximum(linesClip.size());
}
...
void MainWindow::on_addLinePushButton_clicked()
{
    ...
    const int x2 = ui->x2setLineSpinBox->value();
    const int y2 = ui->y2x1setLineSpinBox->value();
...
void MainWindow::mousePressEvent(QMouseEvent *event)
{
    ...
    case Qt::LeftButton:
        if (left_clicked) {
            ...
            ui->x2setLineSpinBox->setValue(x);
            ui->y2x1setLineSpinBox->setValue(y);
...
void MainWindow::receiveKeyboardModifiers(int &x, int &y)
{
    ...
    if (ui->checkBoxParallel->isChecked())
    {
        int n = ui->spinBoxParallel->value();
        QPoint d1 = linesClip[(n - 1) % linesClip.size()];
        QPoint d2 = linesClip[n % linesClip.size()];
        QPoint d = d2 - d1;
        double m = double(d.x()) / d.y();
        int dx = x - x1;
        int dy = y - y1;
        if (qAbs(dx - d.x()) < qAbs(dy - d.y()))
            x = x1 + dy * m;
        else
            y = y1 + dx / m;
    }
