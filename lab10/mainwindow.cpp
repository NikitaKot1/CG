...
void MainWindow::wheelEvent(QWheelEvent *event)
{
    ...
    const int x = event->position().x() - ui->drawLabel->x();
    const int y = event->position().y() - ui->drawLabel->y();

	if (x < 0 || y < 0 || x > 720 || y > 720)
		return;

    addZoom(event->delta() / 2);
	plot();
}
...
{
	const Function& function = FUNCTIONS[ui->functionComboBox->currentIndex()];
	...
	floatingColumnAlgorithm(painter, function, data, factor);
...
void MainWindow::addZoom(double delta)
{
    factor += delta;
...
{
    double max_factor = double(720) / 3 / max(qAbs(xe), qAbs(xb), qAbs(ze), qAbs(zb));
